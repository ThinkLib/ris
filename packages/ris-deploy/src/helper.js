...
const _getRemoteVersions = (type = 'tags') => {
  ...
  return new Promise((resolve, reject) => {
    ...
    return exec(`git ls-remote --${type}`, { cwd: process.cwd() }, (err, stdout) => {
      spin.close();
      remoteVersions = [];
      if (err) return reject(err);

      const reg = new RegExp(`${prefix}/(\\d+\\.\\d+\\.\\d+)`, 'g');
      let match = reg.exec(stdout.trim());
      const versions = [];
      while (match != null) {
        if (match && match[1]) versions.push(match[1]);
        match = reg.exec(stdout.trim());
      }
...
const _push = (version, isDaily) => new Promise((resolve, reject) => {
  let cmd = '';
  let branchTag = '';
  if (isDaily) {
...
const getCurrentVersion = () => new Promise((resolve, reject) => {
...
const getLatestVersion = () => _getRemoteVersions().then((versions) => {
...
const checkRemoteMaster = (remote = 'origin') => {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(cmd, { cwd: process.cwd(), timeout: 5000 }, (err, stdout) => {
      spin.close();
      if (err) console.log(chalk.magenta('Can\'t find remote master branch or timeout, let\'s continue...'));
...
const sleep = (timeout = 0) => new Promise((resolve) => {
